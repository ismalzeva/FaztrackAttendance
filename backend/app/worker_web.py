...
2. Device binding    -> OPSIONAL: jika HP karyawan sudah mendaftarkan kunci
   publik ECDSA P-256 dan disetujui supervisor, absen ditandatangani.
   Tanpa device binding, absen tetap bisa dengan status REVIEW (verifikasi manual).
"""
import hashlib
import json
import secrets
...
def web_login(body: WebLoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    worker = db.scalar(
        select(Worker).where(
            ...
            Worker.code == body.worker_code.upper(),
    ...
    return envelope(
        {
            "access_token": create_worker_token(worker.id, tenant.id),
            "worker": {"code": worker.code, "name": worker.name},
        },
        request,
    )
...
def device_status(
    ...
    """Status device binding karyawan."""
    binding = db.scalar(
        select(DeviceBinding).where(
            DeviceBinding.tenant_id == ctx.tenant_id,
            DeviceBinding.worker_id == ctx.worker.id,
    ...
    pending = db.scalar(
        ...
        .where(
            DeviceEnrollment.tenant_id == ctx.tenant_id,
            DeviceEnrollment.worker_id == ctx.worker.id,
            DeviceEnrollment.status == EnrollmentStatus.PENDING,
        )
        .order_by(DeviceEnrollment.requested_at.desc())
    )
    return envelope(
        {
            "enrolled": binding is not None,
            ...
            "enrollment_id": pending.id if pending else None,
        },
        request,
    )
...
def today_shift(
    ...
    work_date = work_date_for(tenant)
    schedule = db.scalar(
        select(WorkSchedule).where(
            WorkSchedule.tenant_id == ctx.tenant_id,
            WorkSchedule.worker_id == ctx.worker.id,
    ...
    assignments = db.scalars(
        select(Assignment).where(
            ...
            Assignment.worker_id == ctx.worker.id,
    ...
    events = db.scalars(
        ...
        .where(
            ...
            AttendanceEvent.status.in_(counted),
        )
        ...
        timeline.append(
            {
                ...
                "project_name": projects[e.project_id].name,
                "server_time": e.server_time.isoformat(),
                "status": e.status.value,
                "reason_code": e.reason_code,
                "distance_m": round(e.distance_m, 1),
                "auto": (e.signature or "").startswith("AUTO"),
            }
        )
        if e.event_type == AttendanceType.CHECK_IN and e.status != AttendanceStatus.REJECTED:
            open_shift = e
        if e.event_type == AttendanceType.CHECK_OUT and e.status == AttendanceStatus.VALID:
            open_shift = None
    project_list = [
        {
            "id": p.id,
    ...
    ] if hasattr(Project, "is_active") else [
        {
            "id": p.id,
            "code": p.code,
            "name": p.name,
    ...
    return envelope(
        {
            ...
            "open_shift": {
                ...
                "project_name": projects[open_shift.project_id].name,
                "since": open_shift.server_time.isoformat(),
            }
            if open_shift
            else None,
            "timeline": timeline,
...
def _open_project_counts(db, ctx, work_date, counted):
    """Map project_id -> jumlah check-in terbuka."""
    rows = db.scalars(
        select(AttendanceEvent)
        .where(
            AttendanceEvent.tenant_id == ctx.tenant_id,
    ...
    for e in rows:
        if e.event_type == AttendanceType.CHECK_IN and e.status != AttendanceStatus.REJECTED:
            m[e.project_id] = m.get(e.project_id, 0) + 1
        elif e.event_type == AttendanceType.CHECK_OUT and e.status == AttendanceStatus.VALID:
            m[e.project_id] = max(0, m.get(e.project_id, 0) - 1)
    return {k: v for k, v in m.items() if v > 0}


@router.post("/challenge")
def web_challenge(
    body: AttendanceChallengeRequest,
    request: Request,
    ctx: WorkerContext = Depends(worker_context),
    db: Session = Depends(get_db),
):
    ...
    counted = [AttendanceStatus.VALID, AttendanceStatus.REVIEW]
    open_checkin = db.scalar(
        select(AttendanceEvent)
        .where(
            AttendanceEvent.tenant_id == ctx.tenant_id,
            AttendanceEvent.worker_id == ctx.worker.id,
            AttendanceEvent.work_date == work_date,
            AttendanceEvent.event_type == AttendanceType.CHECK_IN,
            AttendanceEvent.status.in_(counted),
        )
    ...
    if event_type == AttendanceType.CHECK_IN:
        ...
        already = (
            body.project_id in open_counts
        ) or (
            open_checkin
            and body.project_id == open_checkin.project_id
            and not tenant.allow_multi_checkin
        )
        if already:
            raise HTTPException(409, detail={"code": "ATTENDANCE_ALREADY_RECORDED"})
    elif not _open_project_counts(db, ctx, work_date, counted):
        raise HTTPException(409, detail={"code": "CHECK_IN_REQUIRED"})
    target = body.project_id or (open_checkin.project_id if open_checkin else None)
    if not target:
        raise HTTPException(409, detail={"code": "PROJECT_REQUIRED"})
...
def web_submit(
    ...
    ch = db.scalar(
        select(AttendanceChallenge).where(
            AttendanceChallenge.id == body.challenge_id,
            AttendanceChallenge.tenant_id == ctx.tenant_id,
            AttendanceChallenge.worker_id == ctx.worker.id,
        )
    )
    if (
        not ch
        or ch.used_at
        or ch.expires_at.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc)
    ):
        raise HTTPException(409, detail={"code": "ATTENDANCE_CHALLENGE_INVALID"})
    if (
        ch.event_type.value != body.event_type
        or ch.project_id != body.project_id
        or hashlib.sha256(b64url_decode(body.challenge)).hexdigest() != ch.challenge_hash
    ):
    ...
    if binding and binding.public_key_jwk:
        try:
            verify_signature(
                json.loads(binding.public_key_jwk),
                signed_payload(body),
                body.signature or "",
            )
        except ValueError:
            raise HTTPException(422, detail={"code": "INVALID_ATTENDANCE_SIGNATURE"})
    ...
    if ch.event_type == AttendanceType.CHECK_IN and tenant.allow_multi_checkin:
        for shift in db.scalars(
            ...
            if shift.project_id != ch.project_id and shift.project_id in _open_project_counts(
                db, ctx, work_date, counted
            ):
                db.add(
                    AttendanceEvent(
                        tenant_id=ctx.tenant_id,
                        ...
                        device_binding_id=binding.id if binding else None,
                        challenge_id=None,
                        event_type=AttendanceType.CHECK_OUT,
                        status=AttendanceStatus.VALID,
                        reason_code="AUTO_CHECKOUT_MOVE_SITE",
                        work_date=work_date,
                        ...
                        distance_m=0.0,
                        signature="AUTO:" + (body.signature or ""),
                    )
                )

    status, reason, distance = attendance_status(
        db.get(Project, ch.project_id),
        body.latitude,
        body.longitude,
        body.accuracy_m,
    )
    ...
    return envelope(
        {
            ...
            "distance_m": round(ev.distance_m, 1),
            "project_name": db.get(Project, ch.project_id).name,
