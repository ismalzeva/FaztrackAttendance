...
def assignment_for_today(ctx: WorkerContext,db: Session):
    tenant=db.get(Tenant,ctx.tenant_id); work_date=work_date_for(tenant)
    assignment=db.scalar(select(Assignment).where(Assignment.tenant_id==ctx.tenant_id,Assignment.worker_id==ctx.worker.id,Assignment.work_date==work_date))
    ...
    if not assignment or not schedule: raise HTTPException(409,detail={"code":"NOT_SCHEDULED_TODAY"})
...
def challenge(body: AttendanceChallengeRequest,request: Request,ctx: WorkerContext=Depends(worker_context),db: Session=Depends(get_db)):
    assignment,project,work_date=assignment_for_today(ctx,db)
    ...
    existing=db.scalar(select(AttendanceEvent).where(AttendanceEvent.tenant_id==ctx.tenant_id,AttendanceEvent.worker_id==ctx.worker.id,AttendanceEvent.work_date==work_date,AttendanceEvent.event_type==event_type,AttendanceEvent.status.in_([AttendanceStatus.VALID,AttendanceStatus.REVIEW])))
    if existing: raise HTTPException(409,detail={"code":"ATTENDANCE_ALREADY_RECORDED","event_id":existing.id})
    if event_type==AttendanceType.CHECK_OUT:
        checkin=db.scalar(select(AttendanceEvent).where(AttendanceEvent.tenant_id==ctx.tenant_id,AttendanceEvent.worker_id==ctx.worker.id,AttendanceEvent.work_date==work_date,AttendanceEvent.event_type==AttendanceType.CHECK_IN,AttendanceEvent.status.in_([AttendanceStatus.VALID,AttendanceStatus.REVIEW])))
        if not checkin: raise HTTPException(409,detail={"code":"CHECK_IN_REQUIRED"})
    ...
    return envelope({"challenge_id":item.id,"challenge":b64url_encode(raw),"expires_in":120,"event_type":event_type.value,"project":{"id":project.id,"code":project.code,"name":project.name,"latitude":project.latitude,"longitude":project.longitude,"radius_m":project.geofence_radius_m}},request)
